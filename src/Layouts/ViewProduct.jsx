...
function ViewProduct() {
  ...
  const handleDelete = async (item) => {
    let deleteData = await axios.delete(
      `${import.meta.env.VITE_LOCAL_API}/api/v1/product/deleteproduct/${item}`,
    );
  ...
  let handleUpdate = async () => {
    await axios.post(
      `${import.meta.env.VITE_LOCAL_API}/api/v1/product/editproduct/${getId}`,
      {
        name: names,
        description: des,
        saleprice: salePrice,
  ...
  return (
    <div className="mt-10 relative ">
      ...
      {edit && (
        <div className="p-8 rounded-xl flex flex-col gap-4 border border-slate-200 w-[35%] bg-white absolute top-20 right-10 shadow-2xl z-50 animate-in fade-in zoom-in duration-200">
